...
def compute_a(z):
    ...
    # import decimal to solve overflow problem
    ...
    for i in range(z.shape[0]):
        sum += Decimal(math.e) ** Decimal(z[i, 0])
    # calculate ai
    for i in range(z.shape[0]):
        a_i = Decimal(math.e) ** Decimal(z[i, 0])/sum
        a.append(float(a_i))

    # force a transfer to matrix
    a = np.asmatrix(a)
    a = a.T
...
def compute_dL_dW(dL_dz,dz_dW):
    ...
    for i in range(dL_dz.shape[0]):
        dL_dW[i, :] = np.multiply(dL_dz[i, 0], dz_dW[i, :])
    # force transfer to matrix
...
def train(X, Y, alpha=0.01, n_epoch=100):
    ...
    for _ in xrange(n_epoch):
        ...
        for x,y in zip(X,Y):
            ...
            dL_dW = compute_dL_dW(compute_dL_dz(dL_da,da_dz), dz_dW)
            dL_db = compute_dL_db(compute_dL_dz(dL_da,da_dz), dz_db)
