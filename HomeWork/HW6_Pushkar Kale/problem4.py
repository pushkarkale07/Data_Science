...
def compute_a1(z1):
    ...
    # for each z calculate a
    a1 = np.asmatrix(np.zeros(z1.shape))
    for i in range(z1.shape[0]):
        a1[i, 0] = 1 / (1 + Decimal(math.e) ** Decimal((-z1[i, 0])))
...
def compute_da1_dz1(a1):
    ...
    da1_dz1 = np.asmatrix(np.zeros(a1.shape))
    for i in range(a1.shape[0]):
        da1_dz1[i, 0] = a1[i, 0] * (1 - a1[i, 0])
