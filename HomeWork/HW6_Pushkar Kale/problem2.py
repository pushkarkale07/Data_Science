...
def compute_a(z):
    ...
    # solve the 'Result too large' Overflowerror
    # float cannot hold that big result
    from decimal import Decimal
    # force transfer z to float, avoid the error in check dL_da
    z = float(z)

    a = 1/(1 + Decimal(math.e) ** Decimal((-z)))

    # force transfer a to float
    a = float(a)
...
def compute_L(a,y):
    ...
    # if y = 0, L = -log(1-a)
    # if y = 1, L = -log(a)
    # 10^6 is to solve the problem of log(0)
    if y == 0:
        if a == 1:
            return 10 ** 6
        L = (-1) * math.log(1-a)
    ...
    for i, x in enumerate(Xtest):
        ...
        for item in test:
            if item >= 0:
                ...
                Y.append(0)
        # force transfer the Y to array
        Y = np.asarray(Y)

        for i in range(n):
            P[i, 0] = compute_a(test[i, 0])
